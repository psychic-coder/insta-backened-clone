...
//:id is used to get hold of the id of the post,we like
router.get("/like/post/:id",isLoggedIn, async function (req, res) {
  const post = await postModel.findOne({ _id: req.params.id });
  const user = await userModel.findOne({ username: req.session.passport.user });
  //the below condition is used to check if the user has already liked the post or not
  ////if the user has not liked the post , the below condition works
  //we are searching if the users id is present or not
  //it is searching for the index of user._id
...
router.get("/feed", isLoggedIn, async function (req, res) {
  ...
  //we are getting hold of all the posts
  //we can populate only those fields where we have stored an id
  let posts = await postModel.find().populate("user");
  res.render("feed", {
    footer: true,user, posts,stories: filtered, dater: utils.formatRelativeTime,
  });
});
...
//regex is used is to search the entire mongoose , even if we type  one letter or more 
//we can search the mongoose
...
//userModel.findOneAndUpdate({uniqueValue},dataTobeUpdated,new:True)
...
//we are posting a image or a story
...
//image is the name of the file we upload , it is assingned in edit.ejs, we are updating
router.post("/upload",isLoggedIn,upload.single("image"),
...
router.post("/register", function (req, res) {
  ...
  userModel.register(user, req.body.password).then(function (registereduser) {
    //till the above line the user is registered , in the below line the user stays logged in
...
router.post(
  "/login",
  //the below line means login on the basis of username and password
